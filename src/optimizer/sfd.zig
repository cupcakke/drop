...
pub const KFACBlock = struct {
    ...
    fn computeInverseSqrt(self: *const KFACBlock, M: *const Tensor) !Tensor {
        ...
        var damped = try Tensor.zeros(self.allocator, M.shape.dims);
        defer damped.deinit();

        var damping = @max(self.damping, @as(f32, 1e-6));
        var attempt: usize = 0;
        while (attempt < 8) : (attempt += 1) {
            ...
            while (row < n) : (row += 1) {
                var column: usize = 0;
                while (column < n) : (column += 1) {
                    const symmetric_value = (@as(f64, M.data[row * n + column]) + @as(f64, M.data[column * n + row])) * 0.5;
                    if (!std.math.isFinite(symmetric_value)) break;
                    damped.data[row * n + column] = @floatCast(symmetric_value);
                }
                if (column != n) break;
            }
            if (row == n) {
                var diagonal: usize = 0;
                while (diagonal < n) : (diagonal += 1) {
                    damped.data[diagonal * n + diagonal] += damping;
                }
                const inverse = damped.choleskyInverse(self.allocator) catch |err| switch (err) {
                    error.MatrixNotPositiveDefinite => null,
                    else => return err,
                };
                if (inverse) |value| return value;
            }
            damping = @min(damping * 10.0, @as(f32, 1e12));
        }

        var fallback = try Tensor.eye(self.allocator, M.shape.dims);
        const scale = 1.0 / @max(damping, @as(f32, 1.0));
        fallback.mulScalar(scale);
        return fallback;
    }
};
